...
// Identify LHS target in inputs or formulas
...
// If LHS is an input but not already in advInputs, push it there
...
function renderInputs(st=null) {
  ...
  // If LHS is a formula (not an input), render a disabled field in Advanced showing its computed value
  if (!lhsInput && lhsFormula) {
    ...
    input.value = format2(current[fake.address]);
  }
}
...
function renderResults(st) {
  ...
  MODEL.formulas.forEach(f => {
    ...
    // Hide LHS row from results if it's the target
  ...
  // Also include Design Density if it is an input
...
function run() {
  ...
  renderResults(st);
  renderBOM(st);
  renderInputs(st); // refresh disabled LHS value if formula
}
